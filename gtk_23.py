...
class AddDialog(Gtk.Dialog):
    def __init__(self, *args, **kwargs):
        ...
        categ = Gtk.Label.new("Category:")
        produ = Gtk.Label.new("Product:")
        quan = Gtk.Label.new("Quantity:")
        aver = Gtk.Label.new("Average:")

        grid.attach(categ, 0, 0, 1, 1)
        grid.attach(produ, 0, 1, 1, 1)
        grid.attach(quan, 0, 2, 1, 1)
        grid.attach(aver, 0, 3, 1, 1)
        grid.attach(combobox, 1, 0, 1, 1)
        grid.attach(entry, 1, 1, 1, 1)
        grid.attach(entry1, 1, 3, 1, 1)
        grid.attach(spin, 1, 2, 1, 1)
        grid.attach(check, 1, 4, 1, 1)
        ...
        if product == "" or category is None:
        ...
        model.set(iter2, BUY_IT, buy, QUANTITY, quantity,
                  PRODUCT, product, FRENCH, category, MOYENNE, moy)
...
class AppWindow(Gtk.ApplicationWindow):
    def __init__(self, *args, **kwargs):
        ...
        scrolled_win.set_size_request(570, 230)
        ...
        bouton_remove.connect("clicked", self.remove_products, self.treeview)
    ...
    def cell_edited_prod(self, renderer, path, new_text, treeview):
        if len(new_text) > 0:
            model = treeview.get_model()
            iter3 = model.get_iter_from_string(path)
            if iter3:
                model.set(iter3, PRODUCT, str(new_text))
                

    def cell_edited_fr(self, renderer, path, new_text, treeview):
        if len(new_text) > 0:
            model = treeview.get_model()
            iter1 = model.get_iter_from_string(path)
            if iter1:
                model.set(iter1, FRENCH, str(new_text))
                

    def cell_edited_moy(self, renderer, path, new_text, treeview):
        if len(new_text) > 0:
            model = treeview.get_model()
            iter4 = model.get_iter_from_string(path)
            if iter4:
                model.set(iter4, MOYENNE, str(new_text))
                

    def remove_row(self, ref, model):
        path = ref.get_path()
        iter5 = model.get_iter(path)
        model.remove(iter5)
